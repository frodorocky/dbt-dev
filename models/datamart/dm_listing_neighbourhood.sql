WITH AggregatedData AS (
    ...
    FROM
        {{ ref('facts_listing') }} 
    GROUP BY
        listing_neighbourhood, month, year
)
...
SELECT 
    ...
    CASE 
        WHEN LAG(active_listings) OVER (PARTITION BY listing_neighbourhood ORDER BY year, month) != 0 THEN 
            (active_listings - LAG(active_listings) OVER (PARTITION BY listing_neighbourhood ORDER BY year, month)) * 100.0 / LAG(active_listings) OVER (PARTITION BY listing_neighbourhood ORDER BY year, month)
        ELSE NULL
    END AS percentage_change_active_listings,
    CASE 
        WHEN LAG(inactive_listings) OVER (PARTITION BY listing_neighbourhood ORDER BY year, month) != 0 THEN 
            (inactive_listings - LAG(inactive_listings) OVER (PARTITION BY listing_neighbourhood ORDER BY year, month)) * 100.0 / LAG(inactive_listings) OVER (PARTITION BY listing_neighbourhood ORDER BY year, month)
        ELSE NULL
